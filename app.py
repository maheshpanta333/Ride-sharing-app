...
<style>
    ...
    .hero-banner h1 { color: #ffffff; margin: 0; font-size: 2.1rem; }
    .hero-banner p { color: rgba(255,255,255,0.9); margin: 0.4rem 0 0 0; font-size: 1.05rem; }
    ...
    .sidebar-title { font-size: 1.35rem; font-weight: 700; margin-bottom: 0.1rem; }
    .sidebar-sub { font-size: 0.85rem; opacity: 0.7; margin-bottom: 0.5rem; }
</style>
    ...
    if drivers_df is not None and not drivers_df.empty:
        ...
        if ongoing_trips is not None and not ongoing_trips.empty:
            for _, row in ongoing_trips.iterrows():
                with st.container(border=True):
                    st.markdown(f"**Trip #{row['trip_id']}** to **{row['drop_off']}**")
                    st.caption(f"Price: Rs. {row['price']} | Pickup: {row['pickup_address']}")

                    # Payment method choice and complete transaction
                    pay_method = st.radio(f"Payment Gateway for Trip #{row['trip_id']}", ["esewa", "khalti"], key=f"pay_{row['trip_id']}", horizontal=True)

                    if st.button(f"Complete Trip & Collect Payment", key=f"btn_{row['trip_id']}", type="primary"):
    ...
    if pending_complaints is not None and not pending_complaints.empty:
        with st.container(border=True):
            ...
            if admins_df is not None and not admins_df.empty:
                ...
                if st.button("Submit Official Response", type="primary"):
                    run_query(
